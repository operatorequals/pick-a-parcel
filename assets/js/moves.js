...
async function playTurn({G, playerID}, pauseTimer=3000, finishedPlayerIDs=[]) {
    ...
    if (playerWon){
        ...
        G.player[playerID].phase = GAMEPHASES[6];
        G.player[playerID].message = "You just WON!"

        G.player[nextPlayerID].phase = GAMEPHASES[5];
        G.player[nextPlayerID].message = "You just Lost!"
        // set state to win
        return true;
    }
